...
<body>
    <?php echo "Water World"; ?>
<canvas></canvas>
<script>
      var canvas,ctx,tim,bai;
      canvas = document.getElementsByTagName
      ('canvas')[0];
      ...
      function aaa(){
        ...
        for(c=0;c<pt.lenght;c++){
            p=pt[c];
            p2=[];
            for(a=0;a<p.lenght;a++){
                x=p[a][0];
                y=p[a][1];
                z=p[a][2];
                b=Math.pow(1.5,y/2);
      ...
      ctx.beginPath();
      for(a=0;a<p2.lenght;a++)ctx.lineTo
      (p2[a][0],p2[a][1]);
      ctx.closePath();
      ctx.stroke();
    }
    requestdAnimationFrame(aaa);
}
    function nami(r,s){
